...
void dGPS::getflag(char *str) {  
  ...
  while (Serial1.available()==0){;}   // If nothing available in serial buffer, wait and do nothing.------------------------------------pas sur --------------------------------------
  int index=0;
  if (Serial1.available()>0){         // If data available in serial buffer, then read it
   if (index<2){
      str[index]=Serial1.read();
      index++; }
   else str[index]='\0';
   }
}
...
float dGPS::getdestcoord(){  
  float result;
  while (Serial1.available()==0) {;}      // do nothing until something comes into the serial buffer
  if (Serial1.available()>0){
    result=Serial1.parseFloat();          //read a float value from serial monitor box, upto 2 decimal places                           
    }
  return result;
}
  ...
  if (byteGPS == -1) {           // See if the port is empty yet
    ...
    if (byteGPS==13){            // If the received byte is = to 13, end of transmission 
      ...
      if(bien==6){               // If yes, continue and process the data
          ...
          if (fLat - desLat > 0){       // direction of the vector   direction SUD 
             //fazimuth = 180 + fazimuth;
             if (fazimuth >= 0){
              fazimuth = 180 - fazimuth; 
             }
             else {
               fazimuth = -fazimuth + 180;
             }
          }
          else {   // direction nord
            if (fazimuth < 0){
